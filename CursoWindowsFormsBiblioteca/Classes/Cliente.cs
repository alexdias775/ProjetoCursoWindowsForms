...
namespace CursoWindowsFormsBiblioteca.Classes
{
    public class Cliente
    {
        public class Unit
        {
            ...
            public Unit BuscarFichario(string conexao, string Id) 
            {
                Fichario F = new Fichario(conexao);      
                if (F.status)
                {
            ...
            public Unit BuscarFicharioDB(string conexao, string Id)
            {
                FicharioDB F = new FicharioDB(conexao);
                if (F.status)
                {
            ...
            public Unit BuscarFicharioSQL(string conexao, string Id)
            {
                FicharioSQLServer F = new FicharioSQLServer(conexao);
                if (F.status)
                {
            ...
            {
                //um select retorna uma datable, que por sua vez é constituíta por um conjunto de datarows 
                //então se fez uma função a qual recebe um datarows e retorna uma classe
                //esse processo é necessário para que se possa realizar a escrita do formulário após 
                //o select 
            ...
            public void IncluirFicharioSQLREL()
            {
                ...
                {
                    throw new Exception("Inclusão não permitidade. Indetificador: " + this.Id + ", erro: " + ex.Message);
                }
            }
            ...
            public Unit BuscarFicharioSQLREL(string Id)
            {
                try
                {
                    string SQL = "SELECT * FROM [TB_Cliente1] WHERE Id = '" + Id + "'";
                    var db = new SQLServerClass();
            ...
            public void AlterarFicharioSQLREL(string Id)
            {
                ...
                {
                    throw new Exception("Alteração não permitidade. Indetificador: " + this.Id + ", erro: " + ex.Message);
                }
            }
            ...
            public void ApagarFicharioSQLREL()
            {
                try
                {
                    string SQL = "SELECT * FROM [TB_Cliente1] WHERE Id = '" + this.Id + "'";
                    var db = new SQLServerClass();
                    var Dt = db.SQLQuery(SQL);
                    ...
                    {
                        SQL = "DELETE FROM [dbo].[TB_Cliente1] WHERE [Id] = '" + this.Id + "';";
                        db.SQLCommand(SQL);
                        db.Close();
                    }
                ...
                {
                    throw new Exception("Erro, não foi possível apagar o registro, cujo identificador é: " + this.Id + ", erro: " + ex.Message);
                }
            }
            ...
            public List<List<string>> BuscarFicharioTodosSQLREL()
            {
                ...
                try
                {
                    var SQL = "SELECT * FROM TB_Cliente1";
                    var db = new SQLServerClass();
                    var Dt = db.SQLQuery(SQL);
        ...
        {
            return JsonConvert.DeserializeObject<Unit>(vJson); //convertendo de json para objeto
        }

        public static string SerializedClassUnit(Unit unit)
        {
            return JsonConvert.SerializeObject(unit); //convertendo de objeto para json
        }
    }
}
